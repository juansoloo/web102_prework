...
raisedCard.innerHTML = `<div>$${amountRaised.toLocaleString('en-US')}</div>`
...
gamesCard.innerHTML = `<div>${totalGames}</div>`
...
function filterUnfundedOnly() {
    ...
    let listOfRaisedLessThanGoal = GAMES_JSON.filter((game) => {
        return game.pledged < game.goal;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(listOfRaisedLessThanGoal);
}
...
const displayStr = `A total of ${unfundedCount} games are still unfunded.`
...
let [first, second, ...list] = sortedGames;
...
const secondGame = document.createElement("dv");
